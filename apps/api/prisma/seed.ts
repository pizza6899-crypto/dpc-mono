...
import { seedGamificationConfig, seedLevelDefinitions, seedItemCatalog } from './seeders/gamification-catalog.seeder';
...
async function main() {
  ...
  try {
    ...
    // 프로모션 설정 시딩 처리
    await seedPromotionConfig(prisma);
    console.log('✅ 프로모션 설정 시딩이 완료되었습니다.');

    // 프로모션 캠페인 시딩 처리
    await seedPromotionCampaigns(prisma);
    console.log('✅ 프로모션 캠페인 시딩이 완료되었습니다.');

    // 쿠폰 설정 시딩 처리
    await seedCouponConfig(prisma);
    console.log('✅ 쿠폰 설정 시딩이 완료되었습니다.');

    // 게이미피케이션 설정 시딩 처리
    await seedGamificationConfig(prisma);
    await seedLevelDefinitions(prisma);
    await seedItemCatalog(prisma);
    console.log('✅ 게이미피케이션 전역 설정, 레벨 및 아이템 카탈로그 시딩이 완료되었습니다.');
