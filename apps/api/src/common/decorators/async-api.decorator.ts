...
import type {
  AsyncApiPub as OriginalPub,
  AsyncApiSub as OriginalSub,
} from 'nestjs-asyncapi';

let PubDecorator: typeof OriginalPub =
  () => (target: any, key?: string | symbol, descriptor?: any) => {};
let SubDecorator: typeof OriginalSub =
  () => (target: any, key?: string | symbol, descriptor?: any) => {};
...
if (process.env.NODE_ENV !== 'production') {
  try {
    ...
    console.warn(
      '[AsyncApiWrapper] Failed to load nestjs-asyncapi library. Falling back to dummy decorators.',
    );
  }
}
