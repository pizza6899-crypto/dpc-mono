...
describe('ConcurrencyService', () => {
    ...
    describe('runExclusive', () => {
        it('should acquire lock, run task, and release lock', async () => {
            ...
            const acquireSpy = jest.spyOn(service, 'tryAcquire').mockResolvedValue(true);
        ...
        it('should release lock with failure status if task throws error', async () => {
            ...
            await expect(service.runExclusive('test-task', task)).rejects.toThrow('Task Failed');

            expect(loggerSpy).toHaveBeenCalledWith(expect.stringContaining('작업 실행 중 에러'), expect.any(Error));
            expect(releaseSpy).toHaveBeenCalledWith('test-task', false, 'Task Failed');
        });
    });
});
