...
describe('NodeIdentityService', () => {
    ...
    describe('onModuleInit', () => {
        ...
        it.skip('should throw error if all IDs are occupied', async () => {
            ...
            await expect(initPromise).rejects.toThrow('All Node IDs are currently occupied in Redis.');
    ...
    describe('Heartbeat', () => {
        ...
        it('should attempt re-occupy if ownership is lost', async () => {
            ...
            await jest.advanceTimersByTimeAsync(10000);
            expect(redisService.setLock).toHaveBeenCalledWith('infra:nodes:0', expect.any(String), 30);
        });
        ...
        it('should log fatal error if re-occupy also fails', async () => {
            ...
            const loggerSpy = jest.spyOn((service as any).logger, 'fatal').mockImplementation();
            await jest.advanceTimersByTimeAsync(10000);

            expect(loggerSpy).toHaveBeenCalledWith(expect.stringContaining('stolen! ID collision risk'));
        });
    });
