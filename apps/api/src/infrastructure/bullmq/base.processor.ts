  ...
  implements OnApplicationShutdown
{
  ...
  async process(job: Job<T>): Promise<R> {
    ...
    return this.cls.run(async () => {
      ...
      try {
        this.logger.debug(
          `[Job 시작] ${job.queueName}:${job.name} (ID: ${job.id}, Attempt: ${job.attemptsMade + 1})`,
        );
        ...
        this.logger.log(
          `[Job 완료] ${job.queueName} (ID: ${job.id}, Duration: ${duration}ms)`,
        );
  ...
  async onApplicationShutdown(signal?: string): Promise<void> {
    ...
    this.logger.log(
      `${this.constructor.name} 종료 프로세스 시작... (Signal: ${signal || 'N/A'})`,
    );
    ...
    try {
      ...
      await worker.close();
      this.logger.log(
        `${this.constructor.name} 워커가 안전하게 종료되었습니다.`,
      );
    } catch (error) {
      ...
      if (
        !msg.includes('closed') &&
        !msg.includes('has not yet been initialized')
      ) {
