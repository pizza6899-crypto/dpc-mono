...
export class BullMqSchedulerService implements OnModuleInit {
    ...
    async onModuleInit() {
        ...
        const targetQueues = ALL_BULLMQ_QUEUES.filter(q => (q as QueueConfig).repeatableJobs && (q as QueueConfig).repeatableJobs!.length > 0);
            ...
            try {
                ...
                for (const job of existingJobs) {
                    ...
                    if (!isPatternSame || !isTzSame) {
                        await queue.removeJobScheduler(job.key);
                        this.logger.log(`🔄 Updating scheduler config: [${config.name}] ${job.name} (Old: ${job.pattern}/${job.tz} -> New: ${definedJob.repeat.pattern}/${definedTz})`);
                    }
                }
