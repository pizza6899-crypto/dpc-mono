...
export class SettleDailyCompService {
  ...
  async processPendingSettlementsBatch(
    ...
    batchSize: number,
    processor: (
      batch: Array<{ userId: bigint; currency: ExchangeCurrencyCode }>,
    ) => Promise<void>,
  ) {
    ...
    while (hasMore) {
      const pendings =
        await this.compDailySettlementRepository.findPendingSettlements(
          untilDate,
          skip,
          batchSize,
        );
  ...
  async processSingleSettlement(
    ...
    const secureTotalEarned =
      await this.compDailySettlementRepository.getPendingTotalForUser(
        userId,
        currency,
        untilDate,
      );
    ...
    if (secureTotalEarned.lte(0)) {
      this.logger.log(
        `Comp settlement skipped for user ${userId}: Re-calculated pending total is 0.`,
      );
      return;
    }
    ...
    if (!account) {
      this.logger.warn(
        `Comp settlement skipped for user ${userId}: Account not found.`,
      );
      await this.compDailySettlementRepository.updateStatuses(
        userId,
        currency,
        CompSettlementStatus.SKIPPED,
        untilDate,
      );
      return;
    }
    ...
    try {
      ...
      if (error instanceof CompPolicyViolationException) {
        this.logger.warn(
          `Comp settlement skipped for user ${userId}: ${error.message}`,
        );
    ...
    this.logger.log(
      `Comp Settlement Processed: user=${userId}, amount=${secureTotalEarned}, rewardId=${reward.id}`,
    );
  }
}
