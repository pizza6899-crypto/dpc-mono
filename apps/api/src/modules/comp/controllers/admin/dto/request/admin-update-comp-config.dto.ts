...
export class AdminUpdateCompConfigDto {
    ...
    @ApiPropertyOptional({ description: 'Enable/Disable comp earning / 콤프 적립 활성화 여부' })
    @IsOptional()
    @IsBoolean()
    isEarnEnabled?: boolean;

    @ApiPropertyOptional({ description: 'Enable/Disable comp claiming / 콤프 전환 활성화 여부' })
    @IsOptional()
    @IsBoolean()
    isClaimEnabled?: boolean;

    @ApiPropertyOptional({ description: 'Allow negative balance for rollbacks / 롤백 시 마이너스 잔액 허용 여부' })
    @IsOptional()
    @IsBoolean()
