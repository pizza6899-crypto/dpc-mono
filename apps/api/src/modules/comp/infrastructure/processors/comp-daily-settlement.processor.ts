...
export class CompDailySettlementProcessor extends BaseProcessor<
  UserSettlementData | any,
  void
> {
  ...
  protected async processJob(
    job: Job<UserSettlementData | any>,
  ): Promise<void> {
    const { name } = job;
  ...
  private async handleDispatcherJob(): Promise<void> {
    ...
    this.logger.log(
      `Dispatched total of ${totalDispatched} user settlement jobs.`,
    );
  }

  private async handleUserSettlementJob(
    job: Job<UserSettlementData>,
  ): Promise<void> {
