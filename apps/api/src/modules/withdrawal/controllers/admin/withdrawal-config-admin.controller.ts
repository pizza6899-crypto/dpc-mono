...
export class WithdrawalConfigAdminController {
  ...
  async getCryptoConfigs(
    ...
    const { configs, total } =
      await this.findCryptoConfigsService.execute(query);
    return {
  ...
  async toggleCryptoConfigActive(
    ...
    const config = await this.toggleCryptoConfigActiveService.execute(
      BigInt(id),
    );
    return this.toCryptoResponse(config);
  }
  ...
  async deleteCryptoConfig(@Param('id') id: string): Promise<void> {
  ...
  async deleteBankConfig(@Param('id') id: string): Promise<void> {
    await this.deleteBankConfigService.execute(BigInt(id));
  }

  // ===== Helpers =====

  private toCryptoResponse(
