...
import { BankWithdrawConfig, BankWithdrawConfigAlreadyExistsException } from '../domain';
    ...
    async execute(params: CreateBankConfigParams): Promise<BankWithdrawConfig> {
        ...
        if (existing) {
            ...
            if (!existing.props.deletedAt) {
                throw new BankWithdrawConfigAlreadyExistsException(params.currency, params.bankName);
            }
