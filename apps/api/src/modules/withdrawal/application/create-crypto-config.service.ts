...
import {
  ...
  CryptoWithdrawConfigAlreadyExistsException,
} from '../domain';
...
export class CreateCryptoConfigService {
  ...
  async execute(
    params: CreateCryptoConfigParams,
    ...
    if (existing) {
      ...
      if (!existing.props.deletedAt) {
        throw new CryptoWithdrawConfigAlreadyExistsException(
          params.symbol,
          params.network,
        );
      }
      ...
      existing.update({
        ...
        maxWithdrawAmount: params.maxWithdrawAmount
          ...
          : null,
        autoProcessLimit: params.autoProcessLimit
