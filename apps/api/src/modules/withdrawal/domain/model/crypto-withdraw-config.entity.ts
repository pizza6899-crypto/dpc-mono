...
export class CryptoWithdrawConfig {
  ...
  static rehydrate(
    id: bigint,
    props: CryptoWithdrawConfigProps,
  ...
  static create(
    id: bigint,
    props: Omit<CryptoWithdrawConfigProps, 'updatedAt' | 'deletedAt'>,
  ): CryptoWithdrawConfig {
  ...
  validateAmount(amount: Prisma.Decimal): { valid: boolean; reason?: string } {
    ...
    if (
      this.props.maxWithdrawAmount &&
      amount.gt(this.props.maxWithdrawAmount)
    ) {
      return {
        valid: false,
