...
export class WithdrawalDetail {
    ...
    static createCrypto(params: {
        ...
        const now = new Date();
        const id = params.snowflakeService.generate(now);

        return new WithdrawalDetail(id, {
    ...
    cancel(): void {
        if (!this.canBeCancelled()) {
            throw new WithdrawalCannotBeCancelledException(this.id, this.props.status);
        }
    ...
    markProcessing(): void {
        ...
        if (!isAllowed) {
            throw new InvalidWithdrawalStatusException(
                this.id,
                this.props.status,
                [WithdrawalStatus.PENDING, WithdrawalStatus.PENDING_REVIEW],
            );
        }
        this.props.status = WithdrawalStatus.PROCESSING;
        this.props.updatedAt = new Date();
    ...
    complete(transactionHash?: string): void {
        ...
        if (!isAllowed) {
            throw new InvalidWithdrawalStatusException(
                this.id,
                this.props.status,
                [WithdrawalStatus.PROCESSING, WithdrawalStatus.SENDING],
            );
        }
    ...
    updateProviderMetadata(metadata: Record<string, unknown>): void {
        this.props.providerMetadata = { ...this.props.providerMetadata, ...metadata };
        this.props.updatedAt = new Date();
    }
}
