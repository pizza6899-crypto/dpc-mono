...
const queueConfig = getQueueConfig(
  BULLMQ_QUEUES.TIER_EVALUATOR.USER_EVALUATION,
);

@Processor(queueConfig.processorOptions, queueConfig.workerOptions)
export class TierUserEvaluationProcessor extends BaseProcessor<
  TierEvaluationJobPayload,
  void
> {
  ...
  protected async processJob(
    job: Job<TierEvaluationJobPayload>,
  ): Promise<void> {
    const { data: payload } = job;

    if (
      !payload ||
      payload.type !== TierEvaluationJobType.EVALUATE_USER ||
      !payload.data?.userId
    ) {
      this.logger.warn(
        `Invalid or missing payload for user evaluation: ${JSON.stringify(payload)}`,
      );
      return;
    }
    ...
    try {
      ...
      await this.evaluateUserTierService.evaluateUser(userId, allTiers);
    } catch (error) {
