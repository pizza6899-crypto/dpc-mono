...
export class UserTierAdminResponseDto {
  ...
  @ApiProperty({
    nullable: true,
    description:
      'Date when the last promotion bonus was received / 마지막 승급 보너스 수령 일시',
  })
  ...
  @ApiProperty({
    enum: UserTierStatus,
    description:
      'Current user tier status / 현재 유저 티어 상태 (ACTIVE, GRACE, LOCKED)',
  })
  status: UserTierStatus;

  @ApiProperty({
    nullable: true,
    description: 'End date of the downgrade grace period / 강등 유예 종료 일시',
  })
  downgradeGracePeriodEndsAt: Date | null;

  @ApiProperty({
    description: 'Date of the last tier change / 마지막 티어 변경 일시',
  })
  ...
  @ApiProperty({
    description:
      'Whether the user is eligible for promotion bonuses / 승급 보너스 지급 대상 여부',
  })
  isBonusEligible: boolean;

  @ApiProperty({
    nullable: true,
    description: 'Next scheduled evaluation date / 다음 심사 예정 일시',
  })
  nextEvaluationAt: Date | null;

  @ApiProperty({
    nullable: true,
    description: 'Admin note for the user tier / 관리자 메모',
  })
  note: string | null;
  ...
  @ApiProperty({
    nullable: true,
    description:
      'Date when the downgrade warning was issued / 강등 경고 발생 일시',
  })
  downgradeWarningIssuedAt: Date | null;

  @ApiProperty({
    nullable: true,
    description: 'Target tier ID if demoted / 강등 시 대상 티어 ID',
  })
  downgradeWarningTargetTierId: string | null;

  @ApiProperty({
    nullable: true,
    description: 'Target tier name if demoted / 강등 시 대상 티어 이름',
  })
  downgradeWarningTargetTierName: string | null;

  @ApiProperty({
    description:
      'Final effective benefits including overrides / 오버라이드가 반영된 실질 혜택 정보',
  })
