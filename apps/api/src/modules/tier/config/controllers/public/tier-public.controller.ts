...
export class TierPublicController {
  ...
  @ApiOperation({
    ...
    description:
      'Returns a list of all tiers translated into the requested language. Fallback to English if no language is specified. / 요청한 언어로 번역된 전체 티어 목록을 반환합니다. 언어가 지정되지 않으면 기본값을 사용합니다.',
  })
  ...
  async getTiers(
    ...
    if (query.code) {
      tiers = tiers.filter(
        (t) => t.code.toUpperCase() === query.code!.toUpperCase(),
      );
    }
  ...
  private mapToResponseDto(tier: Tier, lang: Language): TierPublicResponseDto {
    const translation =
      tier.translations.find((t) => t.language === lang) ||
