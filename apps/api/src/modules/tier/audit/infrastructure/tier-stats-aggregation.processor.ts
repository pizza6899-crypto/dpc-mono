  ...
  protected async processJob(job: Job<any>): Promise<void> {
    ...
    try {
      ...
      const tierMap = new Map<
        string,
        { total: number; maintained: number; grace: number }
      >();

      stats.forEach((s) => {
        const tid = s.tierId.toString();
        const current = tierMap.get(tid) || {
          total: 0,
          maintained: 0,
          grace: 0,
        };
      ...
      for (const tier of allTiers) {
        const counts = tierMap.get(tier.id.toString()) || {
          total: 0,
          maintained: 0,
          grace: 0,
        };
      ...
      this.logger.log(
        `Hourly tier stats aggregation job dispatched. (Total Tiers: ${allTiers.length}, Processed: ${processedCount})`,
      );
    } catch (error) {
