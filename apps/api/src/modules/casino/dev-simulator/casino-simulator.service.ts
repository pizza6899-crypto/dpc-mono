...
export class CasinoSimulatorService {
  ...
  async simulateRound(
  ...
  ): Promise<SimulateRoundResponseDto> {
    const { userId, gameId, betAmount, winAmount, currency, providerCode } =
      dto;
    const logs: string[] = [];

    logs.push(
      `[Start] User=${userId}, Game=${gameId}, Bet=${betAmount}, Win=${winAmount}`,
    );
    ...
    let session = await this.findSessionService.findRecent(
      BigInt(userId),
      aggregatorType,
    );
    ...
    // 2. ID 생성
    // 2. ID 생성
    const roundId = this.snowflakeService.generate().id.toString();
    const betTxId = this.snowflakeService.generate().id.toString();
    // winTxId는 Bet 이후에 생성 (시간 역전 방지)

    // 3. Bet 실행
    const betTime = new Date();
    const provider = providerCode
      ? (providerCode as GameProvider)
    ...
    // 4. Win 실행
    const winTxId = this.snowflakeService.generate().id.toString();
