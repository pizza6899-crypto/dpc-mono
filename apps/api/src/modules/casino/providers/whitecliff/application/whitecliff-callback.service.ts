...
export class WhitecliffCallbackService {
  ...
  async credit(
    ...
    try {
      const { user_id, sid, amount, txn_id, round_id, game_id, credit_time, prd_id, is_cancel, desc } = body;
      ...
      if (!session) {
        this.logger.error(`❌ Credit API - Session Not Found: sid=${sid}, user_id=${user_id}`);
        return { status: 0, balance: 0, error: 'INVALID_USER' };
      }
  ...
  async bonus(
    ...
    const verification = this.verifyRequest(body, ['user_id', 'amount', 'txn_id', 'type'], secretKey);
    if ('error' in verification) return verification.error;
    const { currency } = verification;

    try {
      const { user_id, sid, amount, txn_id, round_id, game_id, prd_id, type } = body;
      ...
      if (!session) {
        this.logger.error(`❌ Bonus API - Session Not Found: sid=${sid}, user_id=${user_id}`);
        return { status: 0, balance: 0, error: 'INVALID_USER' };
      }
  ...
  validateSecretKey(secretKey: string) {
    ...
    const currency = this.whitecliffMapperService.convertWhitecliffCurrencyToGamingCurrency(config.currency);
  ...
  private async handleError(
    ...
    switch (errorCode) {
      case CasinoErrorCode.INSUFFICIENT_FUNDS:
        return { status: 0, balance: currentBalance, error: 'INSUFFICIENT_FUNDS' };
      case CasinoErrorCode.INVALID_USER:
      case CasinoErrorCode.USER_BALANCE_NOT_FOUND:
        return { status: 0, balance: 0, error: 'INVALID_USER' };
      case CasinoErrorCode.DUPLICATE_DEBIT:
        return { status: 0, balance: currentBalance, error: 'DUPLICATE_TRANSACTION' };
      case CasinoErrorCode.INVALID_TXN:
        return { status: 0, balance: currentBalance, error: 'TRANSACTION_NOT_FOUND' };
      default:
