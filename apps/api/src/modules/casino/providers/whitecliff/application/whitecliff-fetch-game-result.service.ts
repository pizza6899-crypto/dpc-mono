    ...
    async execute(gameRoundId: bigint, startedAt: Date): Promise<void> {
        return this.cls.run(async () => {
            ...
            try {
                ...
                // 에러 응답 처리
                if ('error' in result) {
                    this.logger.warn(`Failed to fetch Whitecliff game result: ${result.error} (Round: ${gameRoundId})`);
                    return;
                }
                ...
                this.logger.error(`Error fetching Whitecliff game result for round ${gameRoundId}`, error);
