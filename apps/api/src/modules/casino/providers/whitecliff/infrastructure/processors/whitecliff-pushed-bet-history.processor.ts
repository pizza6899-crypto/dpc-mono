...
export class WhitecliffPushedBetHistoryProcessor extends BaseProcessor<any, void> {
    protected readonly logger = new Logger(WhitecliffPushedBetHistoryProcessor.name);
    ...
    protected async processJob(job: Job<any>): Promise<void> {
        ...
        if (name === BULLMQ_QUEUES.CASINO.WHITECLIFF_HISTORY.repeatableJobs![0].name) {
    ...
    private async handleHistoryUpdate() {
        ...
        const endDate = nowUtcMinus({ seconds: 30 }).toISOString().slice(0, -5) + 'Z';
        ...
        for (const config of whitecliffConfigs) {
            ...
            const gameCurrency = this.whitecliffMapperService.convertWhitecliffCurrencyToGamingCurrency(
                config.currency,
            );

            // Evolution Live Casino 게임들
            const liveCasinoProductIds = gameCurrency === 'KRW' ? [31] : gameCurrency === 'IDR' ? [29] : [1];

            for (const prdId of liveCasinoProductIds) {
                const lastProcessedTime = await this.getLastProcessedTime(gameCurrency, prdId);
                const initialStartDate = lastProcessedTime ? new Date(lastProcessedTime) : nowUtcMinus({ minutes: 120 });

                const maxGapMs = 2 * 60 * 60 * 1000;
                const currentEnd = new Date(Math.min(initialStartDate.getTime() + maxGapMs, endDateObj.getTime()));
    ...
    private async processPushedBetHistoryForProduct({
        ...
        try {
            ...
            const pushedBetHistory = (response as PushedBetHistoryResponse).data?.filter(
            ...
            const roundsToMarkChecked = pendingRounds.filter((r) => !pushedRoundIds.has(r.aggregatorRoundId));
            ...
            this.logger.error(error, `Failed to process pushed bet history for product ${prdId}`);
            return false;
        }
    }

    private async getLastProcessedTime(gameCurrency: GamingCurrencyCode, prdId: number): Promise<string | null> {
        return await this.cacheService.get<string>(
            CACHE_CONFIG.CASINO.WHITECLIFF_PROCESSED_TIME(gameCurrency, prdId)
        );
    }

    private async setLastProcessedTime(gameCurrency: GamingCurrencyCode, prdId: number, processedTime: string): Promise<void> {
