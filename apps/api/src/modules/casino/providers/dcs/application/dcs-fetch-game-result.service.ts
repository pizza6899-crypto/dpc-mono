...
export class DcsFetchGameResultService {
    ...
    async execute(gameRoundId: bigint, startedAt: Date): Promise<void> {
        return this.cls.run(async () => {
            const round = await this.gameRoundRepository.findById(gameRoundId, startedAt);

            if (!round) {
                this.logger.error(`GameRound not found: ${gameRoundId} (startedAt: ${startedAt})`);
                return;
            }

            if (round.aggregatorType !== GameAggregatorType.DC) {
                this.logger.warn(`Invalid aggregator type for round ${gameRoundId}: ${round.aggregatorType}`);
                return;
            }
            ...
            try {
                ...
                if (result.code !== DcsResponseCode.SUCCESS) {
                    this.logger.warn(`Failed to fetch DCS game replay: ${result.msg} (Round: ${gameRoundId})`);
                    return;
                }
                ...
                if (meta) {
                    ...
                    await this.gameRoundRepository.updateResultMeta(round.id, round.startedAt, meta);
                    this.logger.log(`Updated game result metadata for round ${gameRoundId}`);
                }

            } catch (error) {
                this.logger.error(`Error fetching DCS game result for round ${gameRoundId}`, error);
