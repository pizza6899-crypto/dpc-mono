...
export class GameAdminController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves a paginated list of all casino games with administrative details. / 관리자 측면에서의 모든 카지노 게임 목록을 페이징하여 조회합니다.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Updates an existing game identified by its internal ID. Game creation is only available via aggregator sync. / 내부 ID로 식별된 기존 게임 정보를 수정합니다. 게임 생성은 애그리게이터 동기화로만 가능합니다.',
  })
  ...
  async update(
    @Param('id') id: string,
    @Body() dto: UpdateGameAdminRequestDto,
  ) {
    const game = await this.updateGameService.execute({
      ...
      contributionRate: dto.contributionRate
        ? new Prisma.Decimal(dto.contributionRate)
        : undefined,
    });
