...
export class GameTranslationUpdateDto {
  @ApiPropertyOptional({
    enum: Language,
    description: '언어 코드 / Language code',
  })
  @IsEnum(Language)
  language: Language;
...
export class UpdateGameAdminRequestDto {
  ...
  @ApiPropertyOptional({
    description: 'RTP (Return To Player) 백분율 / RTP percentage',
  })
  @IsOptional()
  @IsNumber()
  rtp?: number;
  ...
  @ApiPropertyOptional({
    description: '변동성 (LOW, MEDIUM, HIGH, VERY_HIGH) / Volatility',
  })
  @IsOptional()
  @IsString()
  volatility?: string;
  ...
  @ApiPropertyOptional({
    description: '테이블 ID (라이브 게임용) / Table ID (for live games)',
  })
  @IsOptional()
  @IsString()
  tableId?: string;
  ...
  @ApiPropertyOptional({
    description: '사용자 노출 여부 / Visibility to users',
  })
  @IsOptional()
  @IsBoolean()
  isVisible?: boolean;
  ...
  @ApiPropertyOptional({
    ...
    description: '번역 정보 / Translations',
  })
  @IsOptional()
  @IsArray()
