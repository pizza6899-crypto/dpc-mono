...
import { AddGamesToCategoryRequestDto, RemoveGamesFromCategoryRequestDto } from '../controllers/admin/dto/request/manage-category-games.request.dto';
...
export class AddGamesToCategoryService {
    ...
    @Transactional()
    async execute(categoryId: bigint, dto: AddGamesToCategoryRequestDto): Promise<void> {
...
export class RemoveGamesFromCategoryService {
    ...
    @Transactional()
    async execute(categoryId: bigint, dto: RemoveGamesFromCategoryRequestDto): Promise<void> {
        await this.repository.getById(categoryId); // 카테고리 존재 확인
        const gameIds = dto.gameIds.map(id => BigInt(id));
