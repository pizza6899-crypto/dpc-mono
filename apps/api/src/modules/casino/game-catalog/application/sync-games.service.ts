...
import {
  AggregatorGameDto,
  AGGREGATOR_CODE_MAP,
} from '../../aggregator/ports/aggregator-game.dto';
...
export class SyncGamesService {
  ...
  private async syncAggregator(
    ...
    try {
      ...
      if (!aggregator) {
        this.logger.error(
          `Aggregator not found in DB for type ${type} (code: ${aggregatorCode})`,
        );
        return;
      }
      ...
      for (const gameDto of games) {
        ...
        try {
          ...
          this.logger.error(
            `Failed to sync game ${gameDto.gameCode}: ${e.message}`,
          );
          stats.failed++;
        }
      }
      ...
      if (games.length > 0) {
        await this.deactivateMissingGames(
          aggregator.id,
          fetchedGameCodes,
          stats,
        );
        ...
        this.logger.warn(
          `No games fetched for ${type}. Skipping deactivation logic to prevent accidental wipeout.`,
        );
      }
    } catch (e) {
  ...
  private async deactivateMissingGames(
    ...
    this.logger.warn(
      `Found ${gamesToDeactivate.length} games that are no longer in the aggregator list. Deactivating...`,
    );
    ...
    if (gamesToDeactivate.length <= 10) {
      this.logger.log(
        `Deactivating games: ${gamesToDeactivate.map((g) => `${g.code}(${g.externalGameId})`).join(', ')}`,
      );
    }
  ...
  private async resolveProvider(
    ...
    // 2. Enum으로 DB 조회
    let provider = providerCodeEnum
      ? await this.tx.casinoGameProvider.findFirst({
          where: {
            aggregatorId,
