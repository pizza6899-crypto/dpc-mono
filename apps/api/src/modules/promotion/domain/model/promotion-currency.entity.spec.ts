...
describe('PromotionCurrency Entity', () => {
    const createCurrency = (overrides: Partial<Parameters<typeof PromotionCurrency.fromPersistence>[0]> = {}) => {
    ...
    describe('validateMinDepositAmount', () => {
        it('should return true when deposit meets minimum', () => {
            const currency = createCurrency({ minDepositAmount: new Prisma.Decimal(10) });

            expect(currency.validateMinDepositAmount(new Prisma.Decimal(10))).toBe(true);
            expect(currency.validateMinDepositAmount(new Prisma.Decimal(100))).toBe(true);
        });

        it('should return false when deposit below minimum', () => {
            const currency = createCurrency({ minDepositAmount: new Prisma.Decimal(10) });

            expect(currency.validateMinDepositAmount(new Prisma.Decimal(5))).toBe(false);
            expect(currency.validateMinDepositAmount(new Prisma.Decimal(9.99))).toBe(false);
        });
    });

    describe('validateMaxBonusAmount', () => {
        it('should return true when bonus is within limit', () => {
            const currency = createCurrency({ maxBonusAmount: new Prisma.Decimal(100) });

            expect(currency.validateMaxBonusAmount(new Prisma.Decimal(50))).toBe(true);
            expect(currency.validateMaxBonusAmount(new Prisma.Decimal(100))).toBe(true);
        });

        it('should return false when bonus exceeds limit', () => {
            const currency = createCurrency({ maxBonusAmount: new Prisma.Decimal(100) });

            expect(currency.validateMaxBonusAmount(new Prisma.Decimal(150))).toBe(false);
        });

        it('should return true when no max limit set', () => {
            const currency = createCurrency({ maxBonusAmount: null });

            expect(currency.validateMaxBonusAmount(new Prisma.Decimal(10000))).toBe(true);
        });
    });
