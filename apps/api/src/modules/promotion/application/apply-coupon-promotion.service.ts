  ...
  async execute({
    ...
    if (promotion.isDepositRequired) {
      throw new PromotionInvalidConfigurationException(
        'This promotion requires a deposit',
      );
    }
    ...
    const bonusAmount =
      currencySettings.maxBonusAmount ?? new Prisma.Decimal(0);
    if (bonusAmount.lte(0)) {
      throw new PromotionInvalidConfigurationException(
        'Coupon bonus amount must be positive',
      );
    }
    ...
    // 9. 사용 횟수 증가 (Atomic)
    const updatedPromotion = await this.repository.incrementUsageCount(
      promotion.id,
    );
    if (
