...
export class WageringRequirementAdminController {
    ...
    async list(@Query() query: GetWageringRequirementsAdminQueryDto): Promise<any> {
        ...
        const mappedData = paginatedData.data.map(item => ({
            ...
            maxCashConversion: item.maxCashConversion?.toString(),
            convertedAmount: item.convertedAmount?.toString(),
    ...
    async voidRequirement(
        ...
        return {
            ...
            fulfilledAmount: updated.fulfilledAmount?.toString(),
            ...
            autoCancelThreshold: updated.autoCancelThreshold?.toString(),
        };
    }
}
