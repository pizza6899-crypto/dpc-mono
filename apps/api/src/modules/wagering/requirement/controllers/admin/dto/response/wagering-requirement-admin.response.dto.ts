...
export class WageringRequirementAdminResponseDto {
  ...
  @ApiProperty({
    description: 'Principal Amount / 원금 (배수 계산의 기준액)',
    example: '10000',
  })
  ...
  @ApiProperty({
    description: 'Required Amount / 총 목표 롤링 금액',
    example: '100000',
  })
  requiredAmount: string;

  @ApiProperty({
    description: 'Wagered Amount / 현재까지 배팅(기여)된 금액',
    example: '50000',
  })
  wageredAmount: string;

  @ApiProperty({
    description: 'Required Count / 총 목표 롤링 횟수(판수)',
    example: 10,
  })
  ...
  @ApiProperty({
    description: 'Remaining Amount / 남은 목표 금액',
    example: '50000',
  })
  ...
  @ApiProperty({
    description: 'Bonus Amount / 지급된 보너스 금액',
    example: '2000',
  })
  bonusAmount: string;

  @ApiProperty({
    description: 'Initial Fund Amount / 시작 총 자금 잔액',
    example: '12000',
  })
  initialFundAmount: string;

  @ApiProperty({
    description: 'Current Balance / 현재 남은 펀드 잔액',
    example: '11000',
  })
  ...
  @ApiProperty({
    description: 'Real Money Ratio / 초기 레알머니 기여 비율',
    example: '0.8333',
  })
  realMoneyRatio: string;

  @ApiProperty({
    description: 'Is Forfeitable / 사용자 포기 가능 여부',
    example: true,
  })
  isForfeitable: boolean;

  @ApiProperty({
    description: 'Max Cash Conversion / 최대 현금 전환 가능액',
    nullable: true,
  })
  maxCashConversion: string | null;

  @ApiProperty({
    description: 'Converted Amount / 실제 정산 전환액',
    nullable: true,
  })
  ...
  @ApiProperty({
    description: 'Is Auto Cancelable / 오링 시 자동 취소 허용 여부',
  })
  ...
  @ApiProperty({
    description: 'Last Contributed At / 마지막 기여일',
    nullable: true,
  })
  ...
  @ApiProperty({
    description: 'Cancellation Reason Type / 취소 사유 코드',
    nullable: true,
  })
  cancellationReasonType: string | null;

  @ApiProperty({
    description: 'Cancelled By / 취소 수행자 ID (SYSTEM 또는 Admin ID)',
    nullable: true,
  })
  cancelledBy: string | null;

  @ApiProperty({
    description: 'Balance At Cancellation / 취소 당시 잔액',
    nullable: true,
  })
  balanceAtCancellation: string | null;

  @ApiProperty({
    description: 'Forfeited Amount / 몰수된 보너스 금액',
    nullable: true,
  })
