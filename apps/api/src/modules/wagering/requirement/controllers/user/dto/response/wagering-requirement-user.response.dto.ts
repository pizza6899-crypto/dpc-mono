...
export class WageringRequirementUserResponseDto {
    ...
    @ApiProperty({ description: 'Wagered Amount / 현재까지 베팅한(기여된) 롤링 금액', example: '50000' })
    wageredAmount: string;

    @ApiProperty({ description: 'Remaining Amount / 남은 롤링 금액', example: '50000' })
    remainingAmount: string;

    @ApiProperty({ description: 'Required Count / 총 요구 게임 판수 (횟수 롤링 전용)', example: 10 })
    requiredCount: number;

    @ApiProperty({ description: 'Wagered Count / 현재까지 베팅한 게임 판수', example: 5 })
    wageredCount: number;
