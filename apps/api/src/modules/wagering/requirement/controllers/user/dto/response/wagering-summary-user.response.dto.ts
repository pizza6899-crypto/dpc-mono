...
export class WageringSummaryUserResponseDto {
    ...
    @ApiProperty({ description: 'Total remaining wagering amount / 총 남은 롤링 금액', example: '50000' })
    totalRemainingAmount: string;

    @ApiProperty({ description: 'Total required wagering amount / 총 요구 롤링 금액', example: '100000' })
    totalRequiredAmount: string;

    @ApiProperty({ description: 'Overall progress rate (0-100) / 전체 달성률', example: 50 })
