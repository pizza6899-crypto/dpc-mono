...
export class UserConfigAdminController {
  ...
  @ApiOperation({
    ...
    description:
      'Retrieves the system-wide user registration policies, required fields, and validation rules. / 시스템 전역 사용자 가입 정책, 필수 필드 여부 및 유효성 검사 규칙을 조회합니다.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Updates the global user registration policies and validation regex. This affects the behavior of the public registration forms. / 시스템 전역 사용자 가입 정책 및 정규식 설정을 변경합니다. 이 설정은 공개 회원가입 폼의 동작에 즉시 영향을 미칩니다.',
  })
  @AuditLog({
  ...
  async updateConfig(
    ...
    const config = await this.updateUserConfigAdminService.execute(
      admin.id,
      dto,
    );
