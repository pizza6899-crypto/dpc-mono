...
export class UserConfigResponseDto {
    ...
    @ApiProperty({ enum: UserStatus, description: 'Default Status / 가입 시 기본 상태' })
    defaultStatus: UserStatus;

    @ApiProperty({ description: 'Max Daily Signup Per IP / IP당 일일 최대 가입 수' })
    maxDailySignupPerIp: number;

    @ApiProperty({ enum: LoginIdType, description: 'Login ID Type / 로그인 아이디 타입' })
    ...
    @ApiProperty({ enum: OAuthProvider, isArray: true, description: 'Allowed OAuth Providers / 허용된 소셜 로그인 제공자' })
    allowedOAuthProviders: OAuthProvider[];

    @ApiProperty({ nullable: true, description: 'Login ID Email Regex / 이메일 정규식' })
    loginIdEmailRegex: string | null;

    @ApiProperty({ nullable: true, description: 'Login ID Phone Number Regex / 휴대폰 번호 정규식' })
    loginIdPhoneNumberRegex: string | null;

    @ApiProperty({ nullable: true, description: 'Login ID Username Regex / 일반 아이디 정규식' })
    loginIdUsernameRegex: string | null;

    @ApiProperty({ nullable: true, description: 'Nickname Regex / 닉네임 정규식' })
    nicknameRegex: string | null;

    @ApiProperty({ nullable: true, description: 'Password Regex / 비밀번호 정규식' })
    passwordRegex: string | null;

    @ApiProperty({ enum: ExchangeCurrencyCode, description: 'Default Primary Currency / 기본 메인 통화' })
    defaultPrimaryCurrency: ExchangeCurrencyCode;

    @ApiProperty({ enum: ExchangeCurrencyCode, description: 'Default Play Currency / 기본 게임 통화' })
