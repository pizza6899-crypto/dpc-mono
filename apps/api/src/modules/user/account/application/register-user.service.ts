...
import {
  ...
  AvailabilityField,
} from './check-availability.service';
...
export class RegisterUserService {
  ...
  async execute(params: RegisterUserParams): Promise<RegisterUserResult> {
    ...
    if (config.maxDailySignupPerIp > 0) {
      const throttleResult = await this.throttleService.checkLimit(
        throttleKey,
        {
      ...
      if (!throttleResult.allowed) {
        throw new ApiException(
          ...
          'Daily signup limit exceeded for this IP.',
        );
      }
    }
    ...
    const { user } = await this.createUserService.execute({
      ...
      phoneNumber:
        config.loginIdType === LoginIdType.PHONE_NUMBER
          ? providedLoginId
          : null,
      passwordHash,
      registrationMethod,
