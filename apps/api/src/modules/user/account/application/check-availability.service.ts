...
export class CheckAvailabilityService {
  ...
  async execute(
    ...
    switch (field) {
      ...
      case AvailabilityField.LOGIN_ID:
        ...
        if (config.loginIdType === LoginIdType.EMAIL)
          ...
          regex = config.loginIdUsernameRegex;
    ...
    if (
      ...
      (field === AvailabilityField.LOGIN_ID &&
        config.loginIdType === LoginIdType.EMAIL)
    ) {
    ...
    if (
      field === AvailabilityField.NICKNAME ||
      (field === AvailabilityField.LOGIN_ID &&
        config.loginIdType === LoginIdType.USERNAME)
    ) {
      try {
