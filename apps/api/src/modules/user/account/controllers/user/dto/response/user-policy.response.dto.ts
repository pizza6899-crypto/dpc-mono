...
export class UserPolicyResponseDto {
  ...
  @ApiProperty({
    description: 'Login ID type / 로그인 ID 타입',
    enum: LoginIdType,
  })
  ...
  @ApiProperty({
    description: 'Login ID email regex / 로그인 ID(이메일) 유효성 검사 식',
    nullable: true,
  })
  ...
  @ApiProperty({
    description:
      'Login ID phone number regex / 로그인 ID(휴대폰) 유효성 검사 식',
    nullable: true,
  })
  loginIdPhoneNumberRegex: string | null;

  @ApiProperty({
    description: 'Login ID username regex / 로그인 ID(이름) 유효성 검사 식',
    nullable: true,
  })
  loginIdUsernameRegex: string | null;

  @ApiProperty({
    description: 'Nickname regex / 닉네임 유효성 검사 식',
    nullable: true,
  })
  nicknameRegex: string | null;

  @ApiProperty({
    description: 'Password regex / 비밀번호 유효성 검사 식',
    nullable: true,
  })
