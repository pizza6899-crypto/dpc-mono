...
export class UserAccountController {
  ...
  @ApiOperation({
    ...
    description:
      'Get current registration policy for dynamic form configuration (required fields, validation regex, etc.). / 회원가입 폼 구성을 위해 현재 설정된 가입 정책(필수 필드, 유효성 검사 정규식 등) 정보를 조회합니다.',
  })
  ...
  @ApiOperation({
    ...
    description:
      'Check if the login ID or nickname is already in use. / 아이디 또는 닉네임의 중복 여부를 확인합니다.',
  })
  @AuditLog({
  ...
  @ApiOperation({
    ...
    description:
      'Create an account using ID and password. The required fields and validation rules follow the settings from "GET /users/policy". / 아이디와 비밀번호를 사용하여 계정을 생성합니다. 필수 입력 항목 및 검증 규칙은 "/users/policy" API의 설정을 따릅니다.',
  })
  @AuditLog({
    type: LogType.AUTH,
    action: 'REGISTER',
  ...
  @ApiOperation({
    ...
    description:
      'Create an account by linking a social account (Google, Apple, etc.). Validation rules follow the settings from "GET /users/policy". / 소셜 계정(Google, Apple 등)을 연동하여 계정을 생성합니다. 검증 규칙은 "/users/policy" API의 설정을 따릅니다.',
  })
  @AuditLog({
    type: LogType.AUTH,
