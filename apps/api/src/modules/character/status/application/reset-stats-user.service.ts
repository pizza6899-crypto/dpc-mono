...
} from '../../master/ports/character-config.repository.port';
import type { CharacterConfigRepositoryPort } from '../../master/ports/character-config.repository.port';
...
import {
  CharacterCurrencyNotSupportedException
} from '../domain/character.exception';
import { CharacterConfigNotFoundException } from '../../master/domain/master.exception';
...
export class ResetStatsUserService {
  constructor(
    ...
    private readonly configRepository: CharacterConfigRepositoryPort,
  ...
  async execute(userId: bigint, currency: ExchangeCurrencyCode): Promise<UserCharacter> {
    ...
    if (!price) {
      throw new CharacterCurrencyNotSupportedException();
    }
