...
import type {
  ForbiddenWordRepositoryPort,
  AiModerationLogRepositoryPort,
} from '../ports/out/moderation-repository.port';
import { AI_MODERATION_PORT } from '../ports/out/ai-moderation.port';
import type {
  AiModerationPort,
  AiModerationResult,
} from '../ports/out/ai-moderation.port';
import {
...
export class ModerationService {
    ...
    if (!options?.skipAi) {
      ...
      if (!result.isAllowed) {
        if ((result.confidence ?? 0) >= this.REJECT_CONFIRM_THRESHOLD) {
          this.logger.warn(
            `AI Moderation Rejected: "${content}" (Reason: ${result.message}, Flagged: ${result.flaggedWords?.join(', ')})`,
          );
          ...
          throw new AiModerationRejectedException(
            result.message,
            result.flaggedWords,
          );
        }
        this.logger.warn(
          `AI rejected but confidence too low (${result.confidence}), allowing as potential false positive: "${content}"`,
        );
      }
    }
  }
  ...
  public async saveModerationLog(
    input: string,
    ...
    try {
      ...
      this.logger.error(
        `[AI-Log] Failed to save moderation log: ${error.message}`,
      );
    }
  }
  ...
  public async learnFromAi(
    ...
    try {
      ...
      flaggedWords.forEach((word) =>
        wordsToBlock.add(word.toLowerCase().trim()),
      );
      ...
      this.logger.log(
        `[Auto-Learn] Successfully added ${forbiddenWords.length} new forbidden words: [${Array.from(wordsToBlock).join(', ')}]`,
      );
    } catch (error: any) {
      this.logger.error(
        `[Auto-Learn] Failed to save AI-detected words: ${error.message}`,
      );
    }
  }
}
