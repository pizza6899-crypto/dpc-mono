...
export class AiModerationAdapter implements AiModerationPort {
  ...
  async check(content: string): Promise<AiModerationResult> {
    const { enabled, accountId, apiToken, model } =
      this.envService.cloudflareAi;

    // 1. 비활성화 또는 필수 설정(ID/Token) 누락 시 즉시 통과 (Fail-safe)
    if (!enabled || !accountId || !apiToken) {
      if (enabled && (!accountId || !apiToken)) {
        this.logger.error(
          '[CloudflareAI] Enabled but missing required configuration (accountId or apiToken)',
        );
      }
      return {
        isAllowed: true,
...
`,
      ...
      if (!data?.success || !data?.result?.response) {
        this.logger.warn(
          `[CloudflareAI] Unexpected response structure: ${JSON.stringify(data)}`,
        );
      ...
      this.logger.debug(
        `[CloudflareAI] Raw AI response data: ${JSON.stringify(data)}`,
      );
      ...
      this.logger.debug(
        `[CloudflareAI] Parsed Content: ${JSON.stringify(parsed)}`,
      );
      ...
      return {
        ...
        flaggedWords: (
          parsed.potential_violations ||
          (parsed as any).potentialViolations ||
          []
        )?.slice(0, 10),
        raw: data,
        durationMs,
      ...
      const durationMs = Date.now() - startTime;
      this.logger.error(
        `[CloudflareAI] API call failed: ${error.message}`,
        error.stack,
      );
  ...
  private parseAiResponse(response: string): {
    ...
    try {
      ...
      this.logger.warn(
        `[CloudflareAI] Failed to parse AI response: ${response}`,
      );
    }
