...
export class EmailWorker extends WorkerHost implements OnApplicationShutdown {
    ...
    async onApplicationShutdown(signal?: string): Promise<void> {
        try {
            ...
            if (worker) {
                await worker.close();
                this.logger.log('EmailWorker closed successfully');
            }
        } catch (error) {
            this.logger.error('Failed to close EmailWorker:', error);
        }
    }
}
