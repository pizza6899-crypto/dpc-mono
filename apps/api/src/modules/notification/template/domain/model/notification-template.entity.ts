...
export class NotificationTemplate {
  ...
  update(params: Partial<CreateTemplateParams>): void {
    ...
    if (params.description !== undefined)
      (this as any).description = params.description ?? null;
    ...
    if (params.variables !== undefined)
      (this as any).variables = params.variables;
  ...
  addTranslation(translation: NotificationTemplateTranslation): void {
    ...
    const exists = this._translations.some(
  ...
  getTranslation(
    locale: Language,
  ): NotificationTemplateTranslation | undefined {
    // 1. 요청된 언어로 번역 조회
