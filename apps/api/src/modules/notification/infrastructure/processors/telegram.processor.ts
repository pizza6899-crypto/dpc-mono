...
export class TelegramProcessor extends BaseProcessor<NotificationJobData, void> {
    ...
    protected async processJob(job: Job<NotificationJobData>): Promise<void> {
        const { data } = job;
        this.logger.debug(`Processing Telegram job ${job.id} for log ${data.logId}`);
        ...
        this.logger.warn(`Telegram sender not implemented yet. Skipping log ${log.id}`);
