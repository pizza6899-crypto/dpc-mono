...
export class WalletAdminDetailResponseDto {
  ...
  @ApiProperty({
    description: 'Adjustment Reason Code / 사유 코드',
...
export class WalletTransactionResponseDto {
  ...
  @ApiProperty({
    description: 'Currency Code / 통화',
    enum: ExchangeCurrencyCode,
  })
  currency: ExchangeCurrencyCode;

  @ApiProperty({
    description: 'Transaction Amount (Absolute value) / 거래 금액 (절대값)',
  })
  amount: string;
  ...
  @ApiProperty({
    description:
      'Detailed Balance Change (Main/Bonus) / 잔액 상세 내역 (메인/보너스 구분)',
  })
  balanceDetail: WalletBalanceDetailResponseDto;

  @ApiProperty({
    description: 'Admin Manual Adjustment Detail / 관리자 수동 조작 상세',
    required: false,
  })
  adminDetail?: WalletAdminDetailResponseDto;

  @ApiProperty({
    description: 'System Automatic Processing Detail / 시스템 자동 처리 상세',
    required: false,
  })
...
export class WalletTransactionHistoryResponseDto {
  @ApiProperty({
    description: 'Transaction List / 트랜잭션 목록',
