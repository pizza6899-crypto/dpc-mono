...
export class UpdateWalletStatusService {
  ...
  async execute({
    ...
    adminId,
    reason,
  }: UpdateWalletStatusParams): Promise<UserWallet> {
    // 1. Lock & Get Wallet
    ...
    if (
      newStatus === UserWalletStatus.FROZEN &&
      prevStatus === UserWalletStatus.FROZEN
    ) {
      throw new WalletStatusException(`Wallet is already frozen`);
    }
    if (
      newStatus === UserWalletStatus.FROZEN &&
      prevStatus === UserWalletStatus.INACTIVE
    ) {
      throw new WalletStatusException(
        `Cannot freeze wallet in ${prevStatus} status`,
      );
    }
    if (
      newStatus === UserWalletStatus.ACTIVE &&
      prevStatus === UserWalletStatus.TERMINATED
    ) {
      throw new WalletStatusException(
        `Cannot activate wallet in ${prevStatus} status`,
      );
    }

    if (
      newStatus === UserWalletStatus.FROZEN &&
      !this.walletPolicy.canFreeze(wallet)
    ) {
      throw new WalletStatusException(
        `Cannot freeze wallet in ${prevStatus} status`,
      );
    ...
    if (
      newStatus === UserWalletStatus.ACTIVE &&
      !this.walletPolicy.canActivate(wallet)
    ) {
      throw new WalletStatusException(
        `Cannot activate wallet in ${prevStatus} status`,
      );
    ...
    // 3. Update Status
    switch (newStatus) {
      case UserWalletStatus.ACTIVE:
        wallet.activate();
        break;
      case UserWalletStatus.FROZEN:
        wallet.freeze();
        break;
      case UserWalletStatus.INACTIVE:
        wallet.deactivate();
        break;
      case UserWalletStatus.READ_ONLY:
        wallet.setReadOnly();
        break;
      case UserWalletStatus.WITHDRAWAL_RESTRICTED:
        wallet.restrictWithdrawal();
        break;
      case UserWalletStatus.TERMINATED:
        wallet.terminate();
        break;
    }
