// src/modules/casino-refactor/aggregator/wc/infrastructure/wc-api.service.ts
...
/**
 * WC (Whitecliff) API 서비스
 * Whitecliff API를 호출하는 서비스
 * WcApiPort 인터페이스를 구현합니다.
 */
@Injectable()
export class WcApiService implements WcApiPort {
  private readonly logger = new Logger(WcApiService.name);
   ...
   * @param gameCurrency 조회할 게임 통화 코드
   ...
   * @param gameCurrency 게임 통화 코드
   * @param lang 언어
   * @param provider 게임 프로바이더
   * @param txn_id 트랜잭션 ID
