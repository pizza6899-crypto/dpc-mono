...
import { UpdateArtifactDrawPricesAdminRequestDto } from '../controllers/admin/dto/request/update-artifact-draw-prices-admin.request.dto';
 ...
 */
@Injectable()
export class UpdateArtifactDrawPricesAdminService {
  constructor(
  ...
  /**
   * 글로벌 유물 정책의 뽑기 비용 테이블을 수정하고 저장
   */
  @Transactional()
  async execute(dto: UpdateArtifactDrawPricesAdminRequestDto): Promise<ArtifactPolicy> {
    ...
    policy.updateDrawPrices(dto.drawPrices);

    // 2. 도메인 정책 검증 (합쳐진 최종 상태 확인)
    this.policyPolicy.validateDrawPrices(policy.drawPrices);
