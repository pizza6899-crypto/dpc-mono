...
export class SupportUserController {
  ...
  @ApiOperation({
    summary: 'Send Support Chat Message / 상담 채팅 메시지 전송',
  })
  ...
  @Post(':roomId/read')
  @ApiOperation({
    summary: 'Mark Support Chat Messages as Read / 상담 채팅 메시지 읽음 처리',
  })
  ...
  async markAsRead(
    ...
    const { id: roomId } = this.sqidsService.decodeAuto(roomIdEncoded);
    const { id: lastReadMessageId } = this.sqidsService.decodeAuto(
