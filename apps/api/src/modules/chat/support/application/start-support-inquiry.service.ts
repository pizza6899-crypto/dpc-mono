...
export class StartSupportInquiryService {
  ...
  async execute(params: StartSupportInquiryParams): Promise<ChatRoom> {
    ...
    await this.advisoryLockService.acquireLock(
      LockNamespace.USER_CHAT_SUPPORT,
    ...
    if (!room) {
      ...
      const nextStatus = this.policy.getStatusForReopening(
    ...
    // 유저 멤버십 확인
    const existingMember = await this.memberRepository.findByRoomIdAndUserId(
      savedRoom.id,
      params.userId,
    );
    if (!existingMember) {
