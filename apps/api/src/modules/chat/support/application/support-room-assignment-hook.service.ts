...
export class SupportRoomAssignmentHookService implements OnWebsocketConnectHook {
    ...
    async onConnect(client: Socket, userId: bigint, isAdmin: boolean): Promise<void> {
        if (!isAdmin) return;

        try {
            // 관리자가 담당자로 지정된 모든 활성 상담방 조회
            const assignedRooms = await this.roomRepository.findActiveRoomsByAdminId(userId);

            for (const room of assignedRooms) {
                this.joinRoom(client, room);
            }
        } catch (error) {
            this.logger.error(`Failed to sync assigned support rooms for admin ${userId}: ${error.message}`);
        }
    }

    private joinRoom(client: Socket, room: { id: bigint; type: ChatRoomType }): void {
