...
import {
  ...
  ChatRoomUnauthorizedException,
  ChatRoomInvalidFileTypeException,
} from '../domain/chat-room.exception';
...
export class SendChatMessageService {
  ...
  async execute(params: SendChatMessageParams): Promise<ChatMessage> {
    ...
    // 멤버십 확인 (채팅 참여 여부 검증)
    if (params.senderId) {
      const member = await this.memberRepository.findByRoomIdAndUserId(
        params.roomId,
        params.senderId,
      );
      if (!member) {
    ...
    await this.roomRepository.updateLastMessageAt(
      params.roomId,
      saved.createdAt,
    );
  ...
  private broadcastMessage(message: ChatMessage, room: ChatRoom): void {
    const encodedRoomId = this.sqidsService.encode(
      room.id,
      SqidsPrefix.CHAT_ROOM,
    );
    ...
    const payload: SocketChatMessageNewPayload = {
      ...
      senderId: message.senderId
        ? this.sqidsService.encode(message.senderId, SqidsPrefix.USER)
        : null,
