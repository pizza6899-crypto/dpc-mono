...
export class ChatRoomMembershipHookService implements OnWebsocketConnectHook {
    ...
    async onConnect(client: Socket, userId: bigint): Promise<void> {
        try {
            ...
            this.logger.error(`Failed to sync chat room memberships for user ${userId}: ${error.message}`);
        }
    }
    ...
    private joinRoom(client: Socket, room: { id: bigint; type: ChatRoomType }): void {
        ...
        this.logger.debug(`Joined membership socket room: ${roomName} (userId: ${client.id})`);
    }
}
