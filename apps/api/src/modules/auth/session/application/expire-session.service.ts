  ...
  async execute(params: ExpireSessionParams): Promise<void> {
    ...
    try {
      ...
      if (!session || session.isTerminated()) {
        this.logger.debug(
          `Session not found or already terminated: ${sessionId}`,
        );
        return;
      }
      ...
      if (userId && session.userId !== userId) {
        this.logger.warn(
          `Session user mismatch: expected ${userId}, found ${session.userId}`,
        );
        return;
      }
      ...
      if (type && session.type !== type) {
        this.logger.debug(
          `Session type mismatch for ${sessionId}: expected ${type}, found ${session.type}. Skipping expire.`,
        );
        return;
      }
      ...
      this.logger.log(
        `Session expired successfully: ${sessionId} (Type: ${session.type}, User: ${session.userId})`,
      );
    } catch (error) {
