...
describe('CredentialUserController', () => {
  ...
  describe('logout', () => {
    it('일반 사용자 로그아웃 시 isAdmin=false로 LogoutService를 호출하고 세션을 종료해야 함', async () => {
      ...
      const mockRequest = {
        logout: jest.fn((callback: (err?: Error) => void) => {
          // logout 콜백 실행 (실제 컨트롤러는 이 콜백 내부에서 session.destroy 호출)
          // 테스트에서는 콜백 실행 후 session.destroy가 호출되는지 확인
          callback();
        }),
        session: {
          destroy: mockDestroy,
      ...
      // 타임아웃 때문에 약간의 지연이 있을 수 있음 (100ms 타임아웃 + 여유)
    ...
    it('req.logout이 실패해도 성공 응답을 반환해야 함', async () => {
      ...
      const mockRequest = {
        logout: jest.fn((callback: (err?: Error) => void) => {
          // 에러를 콜백에 전달 (실제 컨트롤러는 에러 여부와 관계없이 session.destroy 시도)
    ...
    it('req.logout이 없고 req.session.destroy만 있는 경우 세션을 종료해야 함', async () => {
      // Arrange
      mockLogoutService.execute.mockResolvedValue(undefined);
    ...
    it('req.logout과 req.session이 모두 없는 경우에도 성공 응답을 반환해야 함', async () => {
      // Arrange
      mockLogoutService.execute.mockResolvedValue(undefined);
    ...
    it('req.logout 호출 자체가 실패해도 성공 응답을 반환해야 함', async () => {
      // Arrange
      mockLogoutService.execute.mockResolvedValue(undefined);
