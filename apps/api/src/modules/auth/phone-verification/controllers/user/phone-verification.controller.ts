...
export class PhoneVerificationController {
    ...
    @ApiOperation({
        ...
        description: 'Generates and sends a 6-digit verification code to the provided phone number. / 입력된 휴대폰 번호로 6자리 인증번호를 생성하여 발송합니다.',
    })
    @AuditLog({
    ...
    async requestVerification(
        ...
        return this.requestPhoneVerificationService.execute(user.id, dto.phoneNumber);
    }
    ...
    @ApiOperation({
        ...
        description: 'Validates the 6-digit code and registers the phone number to the user profile. / 6자리 인증번호를 검증하고 해당 번호를 유저 프로필에 등록합니다.',
    })
    @AuditLog({
        type: LogType.ACTIVITY,
        category: 'USER',
