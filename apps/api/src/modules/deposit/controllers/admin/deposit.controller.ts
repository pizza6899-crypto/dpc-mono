...
- **REJECTED (거절됨)**: 관리자가 확인 후 승인을 거부한 상태입니다. (성함 불일치, 금액 부족 등)
  ...
  async getDeposits(
    ...
    return {
      data: result.data.map((item) => {
        const p = item.deposit.toPersistence();
        return {
          id: p.id!.toString(),
          userId: p.userId,
          ...
          requestedAmount: p.requestedAmount.toString(),
          actuallyPaid: p.actuallyPaid?.toString() ?? null,
          transactionHash: p.transactionHash,
          processedBy: p.processedBy?.toString() ?? null,
          confirmedAt: p.confirmedAt,
          promotionId: p.promotionId?.toString() ?? null,
          depositorName: p.depositorName,
          depositorBank: p.depositorBank,
          depositorAccount: p.depositorAccount,
  ...
  async getDepositDetail(
    ...
    const p = result.deposit.toPersistence();
    return {
      id: p.id!.toString(),
      userId: p.userId,
