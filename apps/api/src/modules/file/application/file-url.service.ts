...
export class FileUrlService {
    ...
    async getUrl(file: FileEntity, expiresIn = 3600): Promise<string | null> {
        ...
        const cacheConfig = file.accessType === FileAccessType.PUBLIC
            ? CACHE_CONFIG.FILE.URL(file.id)
            : CACHE_CONFIG.FILE.PRIVATE_URL(file.id);

        return this.cacheService.getOrSet(cacheConfig, () => this.generateUrl(file, expiresIn));
    }

    /**
     * 실제 URL을 생성하는 내부 로직입니다.
     */
    private async generateUrl(file: FileEntity, expiresIn: number): Promise<string | null> {
     ...
     */
    async getUrlsByFileIds(fileIds: bigint[], expiresIn = 3600): Promise<Map<string, string>> {
