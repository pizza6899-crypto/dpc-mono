...
import { FileEntity, FileStatus, FileValidationException, FileAccessType, FileConstants } from '../domain';
...
export class CreateFileService {
    ...
    async execute(command: CreateFileCommand): Promise<FileEntity> {
        ...
        if (!FileConstants.ALLOWED_MIME_TYPES.IMAGES.includes(detectedType.mime as any)) {
            throw new FileValidationException(
        ...
        // Image Processing
        if (FileConstants.ALLOWED_MIME_TYPES.IMAGE_OPTIMIZABLE.includes(detectedType.mime as any)) {
            ...
            mimeType = 'image/webp';
            fileExtension = 'webp';
        } else if (FileConstants.ALLOWED_MIME_TYPES.IMAGES.includes(detectedType.mime as any)) {
