...
export class UserRewardResponseDto {
  @ApiProperty({
    description: 'Reward ID (Sqids encoded) / 보상 고유 ID',
    example: 'abc123xy',
  })
  id: string;

  @ApiProperty({
    description: 'Source Type / 지급 소스 유형',
  ...
  @ApiProperty({
    description: 'Wagering Target Type / 롤링 조건 대상',
    enum: WageringTargetType,
  })
  wageringTargetType: WageringTargetType;

  @ApiProperty({
    description: 'Wagering Multiplier / 롤링 배수 (수령 시 적용됨)',
    example: '3.00',
    required: false,
  })
  ...
  @ApiProperty({
    description: 'Wagering Expiry Days / 롤링 만료 조건 기한(일)',
    example: 7,
    required: false,
  })
  ...
  @ApiProperty({
    description: 'Expires At / 만료 예정 시각',
    type: Date,
    required: false,
  })
  expiresAt?: Date | null;
