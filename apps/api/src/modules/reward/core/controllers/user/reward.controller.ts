...
export class RewardController {
  ...
  @ApiOperation({
    ...
    description:
      '내가 지급받은 보상 목록을 조회합니다. (최신순 등 정렬 및 상태 필터 지원)',
  })
  ...
  @ApiStandardResponse(ClaimRewardResponseDto, {
    status: HttpStatus.OK,
    description: '클레임 성공 / Successfully claimed',
  })
  async claimReward(
    @Param('id') encodedId: string,
    @CurrentUser() user: User,
  ): Promise<ClaimRewardResponseDto> {
    const defaultRewardId = this.sqidsService.decode(
      encodedId,
      SqidsPrefix.REWARD,
    );
