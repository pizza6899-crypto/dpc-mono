...
export class GrantRewardRequestDto {
  ...
  @ApiPropertyOptional({
    description: 'Source ID / 출처 ID (BigInt as String)',
  })
  @IsOptional()
  @IsString()
  sourceId?: string;
  ...
  @IsNumberString(
    {},
    { message: 'wageringMultiplier must be a valid numeric string' },
  )
  ...
  @IsNumberString(
    {},
    { message: 'maxCashConversion must be a valid numeric string' },
  )
  ...
  @ApiPropertyOptional({
    description: 'Reward Metadata / 보상 메타데이터 객체',
  })
  @IsOptional()
  @IsObject()
