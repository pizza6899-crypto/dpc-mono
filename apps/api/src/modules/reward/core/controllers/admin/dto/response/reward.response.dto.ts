...
export class AdminRewardResponseDto {
    ...
    @ApiProperty({ enum: RewardSourceType, description: 'Source Type / 출처 타입' })
    ...
    @ApiPropertyOptional({ enum: WageringTargetType, description: 'Wagering Target Type / 롤링 대상 타입' })
    ...
    @ApiPropertyOptional({ description: 'Max Cash Conversion / 최대 현금 전환 기준' })
    maxCashConversion?: string | null;

    @ApiProperty({ description: 'Is Forfeitable / 포기 가능 여부' })
    isForfeitable: boolean;

    @ApiProperty({ enum: RewardStatus, description: 'Status / 상태 (PENDING, CLAIMED, EXPIRED)' })
    status: RewardStatus;
