...
export class RewardAdminController {
  ...
  @ApiOperation({
    ...
    description:
      '시스템 또는 관리자가 전체 유저의 보상 목록을 조회합니다. 유저 ID, 상태 등 다양한 필터링을 지원합니다.',
  })
  @ApiPaginatedResponse(AdminRewardResponseDto)
  async getRewards(
    @Query() query: GetAdminRewardsRequestDto,
  ): Promise<PaginatedData<AdminRewardResponseDto>> {
  ...
  async grantReward(
    @Body() dto: GrantRewardRequestDto,
    ...
    const reward = await this.grantRewardService.execute({
      ...
      wageringMultiplier: dto.wageringMultiplier
        ? new Decimal(dto.wageringMultiplier)
        : undefined,
      wageringExpiryDays: dto.wageringExpiryDays,
      maxCashConversion: dto.maxCashConversion
        ? new Decimal(dto.maxCashConversion)
        : undefined,
  ...
  @ApiOperation({
    ...
    description:
      '지급된 보상을 취소(VOID) 처리합니다. (단, 이미 CLAIMED된 보상은 취소 불가)',
  })
  @AuditLog({
    type: LogType.ACTIVITY,
    category: 'ADMIN',
  ...
  @ApiStandardResponse(AdminRewardResponseDto, {
    status: HttpStatus.OK,
    description: '보상 취소 완료',
  })
  async voidReward(
    @Param('id') id: string,
