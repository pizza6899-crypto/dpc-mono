...
import { CasinoSessionModule } from '../game-session.module';
...
describe('CreateCasinoGameSessionService (Integration)', () => {
  ...
  beforeAll(async () => {
    const module: TestingModule = await Test.createTestingModule({
      imports: [CasinoSessionModule],
    }).compile();

    service = module.get<CreateCasinoGameSessionService>(
  ...
  it('SHOUlD create session with usdExchangeRate', async () => {
    ...
    jest
      .spyOn(exchangeRateService, 'getRate')
      .mockResolvedValue(new Prisma.Decimal(0.00075));
    ...
    const session = await service.execute({
      ...
      compRate: new Prisma.Decimal(0.01),
    });
