...
export class ExchangeRateUpdateProcessor extends BaseProcessor<any, void> implements OnApplicationBootstrap {
    ...
    async onApplicationBootstrap() {
        await this.cls.run(async () => {
            ...
            if (count === 0) {
                this.logger.log('No exchange rate data found. Performing initial sync...');
    ...
    private async updateFiatExchangeRates() {
        ...
        for (const currency of targetCurrencies) {
            try {
                if (currency === ExchangeCurrencyCode.USD || currency === ExchangeCurrencyCode.BTC) {
                    continue;
                }
                ...
                this.logger.debug(`Failed to save exchange rate: USD -> ${currency}`, error);
            }
        }

        await this.exchangeRateService.clearAllCache();
        this.logger.log(`Exchange rate update completed. Success count: ${successCount}`);
    }
}
