...
### 권장 문서 구조 (Template)
1.  **Overview**: 모듈의 핵심 목적과 아키텍처 요약 (Submodule 구조 등)
2.  **Roles & Responsibilities**: 각 서브 모듈이나 컴포넌트의 역할 정의 (R&R Matrix)
3.  **Traffic Control**: "어떤 상황에서 어느 서비스를 호출해야 하는가?" (사용 가이드)
4.  **Anti-Patterns**: 해당 모듈 내에서 금지된 패턴 (예: 순환 참조, 직접 DB 접근 등)

*작성 예시: `apps/api/src/modules/tier/README.md` 참조*
...
*   **DTO 결합도 제거 (DTO Decoupling)**: 
    ...
    *   입력 데이터는 서비스 내부에 정의된 **`Command`** 또는 **`Query`** 인터페이스를 사용합니다.
    *   출력 데이터는 도메인 **`Entity`** 또는 평범한 객체(POJO) 형태의 **`Result`** 인터페이스를 반환합니다.
    *   **컨트롤러의 역할**: Request DTO를 Command/Query로 변환하여 서비스에 전달하고, 서비스의 결과(Result/Entity)를 Response DTO로 매핑합니다.
...
5. [ ] **서비스(Application)가 DTO를 직접 참조하지 않고 내부 인터페이스(Command/Result)를 사용하는가?**
