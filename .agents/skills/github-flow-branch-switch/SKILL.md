...
```bash
new_branch="test/banner-module"

check_exists() {
  local b="$1"
  git show-ref --verify --quiet "refs/heads/$b" && return 0
  git ls-remote --heads origin "$b" | grep -q "$b" && return 0
  return 1
}
...
if ! git remote get-url origin >/dev/null 2>&1; then
  echo "ERROR=origin 원격이 없어 사전 원격 중복 검사를 수행할 수 없습니다."
  exit 1
fi

for s in "${suffixes[@]}"; do
  try="${new_branch}${s}"
  if ! check_exists "$try"; then
    candidate="$try"
    break
  fi
done

if check_exists "$candidate"; then
  echo "ERROR=자동 우회(-v2, -fix) 3회 내 중복 해소 실패"
  exit 1
fi

git switch -c "$candidate" && echo "STATUS=CREATED"
